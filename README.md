...
FAISS vs pgvector vs Others (Comparison)
Why pgvector Wins For Us
Design Decisions
Files to Modify
Files to Create
...
Flask API(api.py)
Redis CacheHit?
Gemini API(Generate Embedding)
Supabase + pgvector(Similarity Search via SQL)
...
Memories and embeddings live in the SAME table. No syncing between two systems. When you INSERT a memory, the embedding goes in the same row. When you DELETE a memory, the embedding is gone too. No orphaned data.

SQL filtering is free. Want to search only memories from "chatgpt" created this week? With FAISS, you'd have to build a custom filter layer. With pgvector, it's just:
...
LIMIT 10;
No persistent disk problem on Render. FAISS stores indexes as files that disappear on Render's free tier. pgvector stores everything in Supabase's managed PostgreSQL — always available, always persistent.

User data deletion is trivial. DELETE FROM memories WHERE uuid = 'user-123' wipes everything — text AND embeddings — in one query. No separate FAISS file to manage.

Scales enough for us. pgvector handles up to ~500K vectors per table comfortably. With 1,000 users × 1,000 memories each, that's only 1M vectors — well within range with proper indexing.
...
Fallback	Keep keyword search as backup	If Gemini API is down, fall back to TF-IDF in 
search_engine.py
6. Files to Modify
Backend (Python)
[MODIFY] 
...
Update 
add_memory()
 to also generate and store an embedding when a new memory is saved
Add an embedding column (type vector(768)) to the 
memories
 table in Supabase
[MODIFY] 
redis_cache.py
...
[MODIFY] 
memoryDB.js
Remove embedding-related stats from 
getStats()
...
VectorMemoryClassifier
 in 
popup.js
 (lines 181-930)	Remove	Local AI that causes freeze. Replaced by backend.
@xenova/transformers import in 
popup.js
Remove	No longer needed — saves ~30MB from bundle.
vectorClassifier.js
Deprecate	Standalone local AI. No longer imported.
search_engine.py
Keep as fallback	TF-IDF backup if Gemini API goes down.
9. New Dependencies
Python (Backend)
Package	Purpose
...
Add embedding vector(768) column to 
memories
 table	Stores embeddings alongside memory text
...
Phase 2: Backend Search API (Days 2-3)
 ...
 Add POST /api/search endpoint to 
api.py
 Add POST /api/embeddings/generate endpoint to 
api.py
...
Phase 4: Auto-Embed + Redis Caching (Days 6-7)
 Modify 
memories.py
 to auto-generate embeddings on 
add_memory()
 Extend 
redis_cache.py
 with search-specific cache functions
